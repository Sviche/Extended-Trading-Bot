...
POSITION_MANAGEMENT = {
    ...
    # Стоплосс (временно отключён)
    'stop_loss_enabled': False,        # ОТКЛЮЧЕНО ВРЕМЕННО
    # 'stop_loss_percent': -70,        # % PnL для стоплосса (отрицательное число, например -70 = -70% PnL)
    # Биржевой SL: ставится как TPSL POSITION ордер сразу после открытия позиции
    # Клиентский фоллбэк: мониторинг PnL каждые monitor_interval_sec (подстраховка)
}
