...
TRADING_SETTINGS = {
    ...
    'batch_size_usd': [200, 300],  # Диапазон размера ВСЕЙ ПАЧКИ в USD [min, max]
    ...
    'leverage': {
        # Формат: 'МОНЕТА': [min, max, шаг] — рандомный leverage
        # Или:    'МОНЕТА': число — фиксированный leverage
        # Если монеты нет в списке — используется конфиг BTC (fallback)
        # Макс. плечи по тирам: Tier1=50x, Tier2=25x, Tier3=10x, Tier4=5x, EUR=100x
        'BTC': [45, 50, 1],
        'ETH': [45, 50, 1],
    },
    ...
    # Воркеры
    'num_workers': 1,                      # Количество параллельных воркеров (по сути количество потоков)
    'account_cooldown_range': [10, 10],  # Время отдыха аккаунта после сделки [min, max] (сек)
...
POSITION_MANAGEMENT = {
    'holding_time_range': [10, 10],  # Время удержания позиции [min, max] (сек)
    'monitor_interval_sec': 60,      # Проверка позиций каждые N сек

    # Стоплосс (ORDER TPSL — прикрепляется к ордеру при создании)
    # SL автоматически создаётся вместе с ордером, без отдельного запроса.
    # Используется OrderTpslType.ORDER — SDK подписывает SL в том же settlement.
    'stop_loss_enabled': True,
    'stop_loss_percent': -70,        # % PnL для стоплосса (отрицательное число, например -70 = -70% PnL)
}
